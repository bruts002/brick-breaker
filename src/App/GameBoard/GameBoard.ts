...
export default class GameBoard {
  ...
  public constructor(
    ...
    this.statusBar = new StatusBar(statusBarWrapperNode, {
  ...
  public init(level: LevelI): void {
    ...
    document.body.addEventListener(
      "keydown",
      this.globalKeyListener.bind(this)
    );
    this.start();
  }
  private buildPlayerAndAI(
  ...
  private getBlock(point: Vector): Block {
    ...
    this.renderedBlocks.some((b: Block, i: number) => {
      ...
      if (
        isNear(point, blockPoint, blockSize) &&
  ...
  private updateBullets(): void {
    ...
    this.bullets.forEach(function(bullet: Bullet, index: number) {
      ...
      for (let indexSpeed = 0; indexSpeed < speed; indexSpeed++) {
        ...
        if (
          nxtPos.x < 0 ||
          nxtPos.x > this.size.width ||
          nxtPos.y < 0 ||
          nxtPos.y >= this.size.height
        ) {
          bulletsToDelete.push(index);
  ...
  private removePredictionLines(): void {
    this.domElement
      .querySelectorAll("line[prediction-line]")
      .forEach(l => this.domElement.removeChild(l));
  }

...
function makeMove(
  ...
  if (ai instanceof Paddle) {
    makePaddleMove(ai, balls, rewards, boardSize);
  } else if (ai instanceof Guy) {
    makeGuyMove(ai, balls, boardSize);
  }
  return balls.reduce(
    (lines: LinePosI[], ball: Ball): LinePosI[] =>
      lines.concat(getLineCoords(ball, boardSize)),
    []
  );
}
...
function catchable(entity: Entity, paddle: Paddle, size: Size): boolean {
  ...
  const movesTillDrop: number = getMovesTillDrop(
    entity,
    size.height,
    paddleSize.height
  );
  const nearestPaddlePoint: number =
    dropPoint.x > paddle.point.x
      ? paddle.point.x + paddleSize.width
      : paddle.point.x;
...
function makePaddleMove(
  ...
  return movePaddleToEntity(paddle, entitiesToGoFor, size);
}
function getHighestEntity(entities: Array<Entity>): Entity {
  let highestEntity: Entity = entities[0];
  entities.forEach(entity => {
    if (highestEntity.point.y > entity.point.y) highestEntity = entity;
  });
...
function getLowestEntity(entities: Array<Entity>): Entity {
  ...
  entities.forEach(entity => {
    if (lowestEntity.point.y < entity.point.y) lowestEntity = entity;
  });
  return lowestEntity;
}
...
function movePaddleToEntity(
  ...
  const nxtNxtPoint: Vector = onlyOneLeft
    ? nxtPoint
    : getEntityDropPoint(entities[1], size, paddleHeight);
  if (
    isLeft(nxtPoint, paddle) ||
    canAndShouldMoveLeft(nxtNxtPoint, nxtPoint, paddle)
  ) {
    paddle.moveLeft();
  } else if (
    isRight(nxtPoint, paddle) ||
    canAndShouldMoveRight(nxtNxtPoint, nxtPoint, paddle)
  ) {
    paddle.moveRight();
  }
...
function canAndShouldMoveLeft(
  candidate: Vector,
  current: Vector,
  paddle: Paddle
): boolean {
  return (
    isLeft(candidate, paddle) &&
    isCollision(
      current,
...
function canAndShouldMoveRight(
  candidate: Vector,
  current: Vector,
  paddle: Paddle
): boolean {
  return (
    isRight(candidate, paddle) &&
    isCollision(
      current,
...
function getEntityDropPoint(
  ...
  const movesTillHit: number = getMovesTillDrop(
    entity,
    boardSize.height,
    paddleHeight
  );

...
function hitsIt(pointX: number, blockStart: number, width: number): boolean {
  const val: number = blockStart + width - pointX;
...
export function isCollision(p1: Vector, p2: Vector, p2Size: Size): boolean {
  return hitsIt(p1.x, p2.x, p2Size.width) && hitsIt(p1.y, p2.y, p2Size.height);
}
